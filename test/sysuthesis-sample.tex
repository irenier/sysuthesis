...
% 提示：样式名中不含年份（如 gbt7714-numeric）默认对应最新的 GB/T 7714 标准。
% 注意：BibTeX 样式后缀已统一为 -numeric 和 -authoryear，旧版 -numerical 后缀会有过时警告。
\sysusetup[bib]
  {
    backend  = bibtex,
    style    = gbt7714-numeric,  % 旧版为 gbt7714-numerical
    resource = sysuthesis-sample
  }
% \sysusetup[bib]
%   {
%     backend  = biblatex,
%     style    = gb7714,  % 旧版为 gb7714-2015
